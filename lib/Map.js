...
Map.prototype.get = function(key) {
  ...
  for (var i = 0; i < this.storage.length; i++) {
    ...
    if (item.key === index) {
      if (item.value != null) {
        // Retrieve the last node in the linked list and its value
        itemValue = item.value.get(item.value.length - 1).value;
...
Map.prototype.remove = function(key) {
  ...
  for (var i = 0; i < this.storage.length; i++) {
    var item = this.storage[i];
    if (item.key === index) {
      item.value = null; // Set the value to null but the key remains
      found = true;
      break;
    }
  }
